...
def gen_train_dataloader(
    ...
    for noisy_data in noisy_data_list:
        if not is_zarr:
            frames = []
            with tifffile.TiffFile(noisy_data) as tiff:
            ...
            if is_raw:
                ...
                if opt.align_data:
                    print("Aligning data...")
                    frames = align_data(
                        frames, opt.alignment_method, **opt.alignment_kwargs
                    )
                frames = frames[:, 0]
            else:
                if opt.align_data:
            ...
            if opt.rolling_mean != 1:
                print(f"Applying rolling mean of {opt.rolling_mean}")
                mean_tiff = np.zeros(
                    (frames.shape[0] - opt.rolling_mean, *frames.shape[1:])
                )
                print(mean_tiff.shape, frames.shape)
                for frame in trange(mean_tiff.shape[0]):
                    mean_tiff[frame] = np.mean(
                        frames[frame : frame + opt.rolling_mean], axis=0
                    )
